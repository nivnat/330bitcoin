...
#include <iostream>
#include <string>
unsigned long hashX(string key,long len)
{
	int sum=0;
...
unsigned long hashY(string key, long len)
{
	int seed=0;
	for(int i=0;i<key.length();i++)
	{
...
unsigned long hashZ(const char* key,long len)
{
	unsigned h=37;
	while(*key)
	{
...
bool myBloom::exists(std::string item) 
{
	...
	if(filter[hashe1]=='0' || filter[hashe2]=='X' ||filter[hashe2]='Z' || filter[hashe2]=='C' )
		return false;

	if(filter[hashe1]=='0' || filter[hashe3]=='X' ||filter[hashe3]='Y' || filter[hashe3]=='A' )
		return false;
